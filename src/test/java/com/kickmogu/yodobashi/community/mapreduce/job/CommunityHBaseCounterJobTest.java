...
public class CommunityHBaseCounterJobTest {
...
//	private CommunityMapReduceConfig mapReduceConfig;
	...
	public void testFilterDeleteAndTimestamp() throws Exception {
		...
		CommunityHBaseCounterJob.execute(applicationContext, new String[]{
		...
		String filePath = "/tmp/hbaseCounter/" + tableNamePrefix + SlipHeaderDO.class.getSimpleName() + "_" + ThreadSafeDateFormat.format(DATE_FORMAT, new Date(baseTimestamp)) + "_count.txt/part-r-00000";
	...
	public void testNomal() throws Exception {
		...
		assertEquals(5, hbaseOperations.scanAll(SlipHeaderDO.class).size());	
		assertKeys("11","12","13", "14", "15");
		CommunityHBaseCounterJob.execute(applicationContext, new String[]{
			"-s", Long.valueOf(baseTimestamp).toString(),
			"-t", hbaseContainer.getMeta().getTableMeta(SlipHeaderDO.class).getTableName(),
			"-f", "cf",
		...
		String filePath = "/tmp/hbaseCounter/" + tableNamePrefix + SlipHeaderDO.class.getSimpleName() + "_" + ThreadSafeDateFormat.format(DATE_FORMAT, new Date(baseTimestamp)) + "_count.txt/part-r-00000";
		List<String> list = readFile(filePath);
		assertEquals(tableNamePrefix + SlipHeaderDO.class.getSimpleName() + "\t5",list.get(0));
		assertEquals(tableNamePrefix + SlipHeaderDO.class.getSimpleName() + "_delete_row\t0",list.get(1));
	}
