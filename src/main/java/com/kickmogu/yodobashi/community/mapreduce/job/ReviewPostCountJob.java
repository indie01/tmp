...
/**
 * レビューの投稿件数を商品マスターに記録するジョブです。
 * @author kamiike
 */
@Service @Lazy @Scope("prototype")
public class ReviewPostCountJob extends AbstractCommunityJob implements InitializingBean {
...
//	@Autowired @Qualifier("default")
	...
	public HBaseTableInputFormat reviewPostCountInputFormat() {
	 ...
	 */
	@Bean @Scope("prototype")
	public SimpleOutputFormat<Text, ProductMasterCountData> reviewPostCountOutputFormat() {
		return new SimpleOutputFormat<Text, ProductMasterCountData>(new ProductMasterCountRecordWriter(
				...
				"reviewPostCount", adminConfigDao.loadScoreFactor()));
	}
	 ...
	 */
	@Bean @Scope("prototype")
	public ReviewPostCountReducer reviewPostCountReducer() {
		return new ReviewPostCountReducer();
	}
	 ...
	 */
	@Bean @Scope("prototype")
	public Job reviewPostCount() throws Exception {
		JobFactoryBean factory = getJobFactoryBean();
		factory.setInputFormat(reviewPostCountInputFormat());
		factory.setReducer(reviewPostCountReducer());
		factory.setOutputFormat(reviewPostCountOutputFormat());
		Job job = factory.getObject();

		job.setMapperClass(ReviewPostCountMapper.class);
	...
	public static int execute(
		...
		return ToolRunner.run(new ReviewPostCountJob(
				).setParentApplicationContext(parentContext),
				new String[]{getTargetDateString(targetDate)});
	}

	/**
	 * レビューの投稿件数集計において、マップ処理を行います。
	 * @author kamiike
	 *
	 */
	public static class ReviewPostCountMapper extends Mapper<ImmutableBytesWritable, Result, Text, LongWritable> {
	...
	/**
	 * レビューの投稿件数集計において、リデュース処理を行います。
	 * @author kamiike
	 */
	public static class ReviewPostCountReducer extends Reducer<Text, LongWritable, Text, ProductMasterCountData> {
