...
public class QuestionScoreUpdateJob extends AbstractCommunityJob {
	...
	public SolrInputFormat<String, QuestionDO> questionScoreUpdateInputFormat() throws ParseException {
		StringBuilder buffer = new StringBuilder();
		buffer.append("status_s:");
		buffer.append(ContentsStatus.SUBMITTED.getCode());
		return new SolrInputFormat<String, QuestionDO>(
				solrOperations,
				buffer.toString(),
				Path.DEFAULT, QuestionDO.class, "registerDateTime_dt");
	}
	 ...
	 */
	@Bean @Scope("prototype")
	public QuestionScoreUpdateMapper questionScoreUpdateMapper() {
		QuestionScoreUpdateMapper mapper = new QuestionScoreUpdateMapper();
	 ...
	 */
	@Bean @Scope("prototype")
	public Job questionScoreUpdate() throws Exception {
		JobFactoryBean factory = getJobFactoryBean();

		factory.setInputFormat(questionScoreUpdateInputFormat());
		factory.setMapper(questionScoreUpdateMapper());
	...
	public static int execute(
		...
		return ToolRunner.run(new QuestionScoreUpdateJob(
				).setParentApplicationContext(parentContext),
				new String[]{getTargetDateString(targetDate)});
	}

	/**
	 * 質問のスコア更新において、マップ処理を行います。
	 * @author kamiike
	 *
	 */
	public static class QuestionScoreUpdateMapper extends Mapper<String, QuestionDO, NullWritable, NullWritable> {
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			try {
				...
				questionService.updateQuestionScoreAndViewCountForBatchBegin(BULK_SIZE);
		...
		public void map(
			...
			questionService.updateQuestionScoreAndViewCountForBatch(targetDate, value, scoreFactor);
		}
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			questionService.updateQuestionScoreAndViewCountForBatchEnd();
		}

		/**
		 * 質問サービスを設定します。
		 * @param questionService 質問サービス
		 */
