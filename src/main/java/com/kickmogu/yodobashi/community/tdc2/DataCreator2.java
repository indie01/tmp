...
import com.kickmogu.yodobashi.community.tdc2.TestDataCreators2.DataCreateContext2;

@SuppressWarnings("rawtypes")
public abstract class DataCreator2<T, I extends TestId.Target> {
	
	private static Log LOG = LogFactory.getLog(DataCreator2.class);
	...
	static final List<String> SKU_LIST = Lists.newArrayList(
		...
		"100000001001260141",
		"100000001001312388",
		"100000001000962314",  // PS3 忍者外伝
		"100000001001253419",   // PS3 Devil May Cry 4
		"100000001001117886"
	);
...
//		}
	...
	public abstract void fillItems(T object, DataCreateContext2 dataCreateContext);
	...
	private static void mainInternal(String[] args) throws Throwable {
        ...
        Map<HBaseTableMeta,List<DataCreator2>> creatorMap = Maps.newTreeMap(new Comparator(){
        ...
        for (String tableName:commandLine.getOptionValue("table").split(",")) {
    		...
    		List<DataCreator2> creators = Lists.newArrayList();
        	for (Class dataCreatorClass:getDataCreatorClasses(tableMeta.getType())) {
        		DataCreator2 dataCreator = (DataCreator2)dataCreatorClass.newInstance();
        ...
        for (List<DataCreator2> dataCreators:creatorMap.values()) {
        	for (DataCreator2 dataCreator:dataCreators) {
...
//		Asserts.isTrue(this.countPerCommunityUser > 0);
	...
	private DataCreateContext2 createDataCreateContext(int num, int communityUserNum) {
		DataCreateContext2 dataCreateContext = new DataCreateContext2();
	...
	static abstract class CreateDataProducerConsumer<T> extends ProducerConsumer<List<T>> {
		...
		DataCreateContext2 dataCreateContext;
		DataCreator2 dataCreator;
		int num;
		int currentCount;
		int bulkSize;
		
		CreateDataProducerConsumer(DataCreator2 dataCreator, DataCreateContext2 dataCreateContext, int num, int producerNum, int consumerNum, int bulkSize) {
	...
	static class HBaseProducerConsumer<T> extends CreateDataProducerConsumer<T> {
		...
		HBaseProducerConsumer(DataCreator2 dataCreator,
				DataCreateContext2 dataCreateContext, int num, int producerNum,
	...
	static class SolrProducerConsumer<T> extends CreateDataProducerConsumer<T> {
		...
		SolrProducerConsumer(DataCreator2 dataCreator,
				DataCreateContext2 dataCreateContext, int num, int producerNum,
				int consumerNum,int bulkSize) {
			super(dataCreator, dataCreateContext, num, producerNum, consumerNum, bulkSize);
		}
	...
	private static List<Class> getDataCreatorClasses(Class type) {
		...
		for (Class creatorClass:TestDataCreators2.class.getClasses()) {
	...
	private static void argError(Options options) {
        ...
        help.setWidth(120);
        help.printHelp(DataCreator2.class.getSimpleName(), options, true);
        System.exit(1);
	}
...
//		return resourceConfig.imageUrl + remoteTargetDirectory + "/" + remoteFileName;
	...
	protected void fillImageHeaderCommonItems(ImageHeaderDO object, DataCreateContext2 dataCreateContext) {
	...
	protected void fillThumbnailImageHeaderCommonItems(ImageHeaderDO object, DataCreateContext2 dataCreateContext) {
		object.setStatus(ContentsStatus.SUBMITTED);
